...
- [[https://junie.jetbrains.com/docs/junie-cli-acp.html][Junie]] (JetBrains)
...
- [[https://github.com/mrcnski/agent-shell-queue-transient][agent-shell-queue-transient]]: A Transient interface for the prompt queue in =agent-shell=.
...
For Junie, install the [[https://junie.jetbrains.com/docs/junie-cli.html][Junie CLI]] and ensure ACP mode is available:

#+begin_src bash
junie --acp true
#+end_src

See https://junie.jetbrains.com/docs/junie-cli-acp.html for details.
...
*** Typing while the agent is busy

When =agent-shell-persistent-prompt-enabled= is set, the shell keeps a writable prompt at the end of the buffer at all times (including agent busy times).

Submitting into a busy agent queues the prompt by default.  =RET= submits via =agent-shell-busy-submit-default-function= (queues), while =M-RET= and =C-u RET= submit via =agent-shell-busy-submit-override-function= (steers).  Steering is covered in the next section.

To disable/hide the persistent prompt, unset the related option:

#+begin_src emacs-lisp
(setopt agent-shell-persistent-prompt-enabled nil)
#+end_src

The viewport's compose buffer is available either way, and submitting from it mid-turn routes the same.
...
- =M-x agent-shell-junie-start-agent= - Start a Junie agent session
...
| agent-shell-busy-submit-default-function      | Function deciding what submitting while the agent is working does.              |
| agent-shell-busy-submit-override-function     | Function M-x agent-shell-submit-override submits through.                       |
...
| agent-shell-junie-acp-command                 | Command and parameters for the JetBrains Junie ACP client.                      |
| agent-shell-junie-default-model-id            | Default Junie model ID.                                                         |
| agent-shell-junie-default-session-mode-id     | Default Junie session mode ID.                                                  |
| agent-shell-junie-environment                 | Environment variables for the Junie ACP client.                                 |
...
| agent-shell-persistent-prompt-enabled         | Whether a shell keeps a writable prompt at the buffer end at all times.         |
...
| agent-shell-session-list-page-limit           | Maximum number of pages to request when listing sessions.                       |
...
|                                                       | agent-shell-junie-start-agent                           | Start an interactive Junie agent shell.                                       |
...
| RET                                                   | agent-shell-submit                                      | Submit the current input to the agent, or queue it while the agent is busy.   |
| M-<return> or M-RET                                   | agent-shell-submit-override                             | Submit the current input through the override route.                          |
...
|                                                       | agent-shell-viewport-compose-send-override              | Send the viewport composed prompt through the override route.                 |
