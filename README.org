...
- [[#minimax-code-mcode][MiniMax Code (MCode)]] (custom configuration)
...
**** MiniMax Code (MCode)
:PROPERTIES:
:CUSTOM_ID: minimax-code-mcode
:END:

Install the [[https://agent.minimax.io][MiniMax Code]] CLI using its [[https://www.npmjs.com/package/@minimax-ai/code][installation instructions]], including the Node.js requirements and npm script permissions, then run =mcode login= in a terminal. Ensure =mcode= is available in Emacs' =exec-path=.

Add this command to your configuration to start MCode through the existing custom-agent API:

#+begin_src emacs-lisp
(defun my-agent-shell-mcode ()
  "Start MiniMax Code through ACP."
  (interactive)
  (require 'agent-shell)
  (agent-shell-start
   :config
   (agent-shell-make-agent-config
    :identifier 'mcode
    :mode-line-name "MCode"
    :buffer-name "MCode"
    :shell-prompt "MCode> "
    :shell-prompt-regexp "MCode> "
...
Run =M-x my-agent-shell-mcode= to start a session. Authentication is handled by the CLI login above.
