...
Steering is always explicit.  =M-x agent-shell-prompt-queue= queues, as it always has.  =M-x agent-shell-prompt-steer= steers, and errors when there is no running turn to steer or the agent cannot steer.  A steer that the agent does not accept falls back to queueing, so the prompt is never lost.
